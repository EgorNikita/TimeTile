...
{
    public Task<Stream> GenerateDefaultIcon(string title, int size = 100)
    {
        return Task.Run(() =>
        {
            var initials = GetInitials(title);
            var backgroundColor = GetColorFromName(title);

            return CreateImage(initials, backgroundColor, size);
        });
    }

    private Stream CreateImage(string initials, SKColor backgroundColor, int size)
    {
    ...
    private string GetInitials(string title)
    {
        ...
        var trimmedTitle = title.Trim().ToUpper();

        var firstInitial = trimmedTitle[0].ToString();
        var lastInitial = trimmedTitle.Length == 1 ? string.Empty : trimmedTitle[1].ToString();
