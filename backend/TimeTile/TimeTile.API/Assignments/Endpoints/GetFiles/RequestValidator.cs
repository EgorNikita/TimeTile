...
{
    public class RequestValidator : AbstractValidator<GetAssignmentFilesEndpoint.Request>
    {
        public RequestValidator(TimetileDbContext db, IInstitutionProvider institutionProvider)
        {
            ...
            RuleFor(x => x.Id)
                ...
                {
                    RuleFor(x => x.Id)
                        .MustBeValidEntityId<GetAssignmentFilesEndpoint.Request, Assignment>(db)
                        .DependentRules(() =>
                        {
                            RuleFor(x => x.Id)
