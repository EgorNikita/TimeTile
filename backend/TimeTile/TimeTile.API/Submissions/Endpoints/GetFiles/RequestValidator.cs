...
{
    public class RequestValidator : AbstractValidator<GetSubmissionFilesEndpoint.Request>
    {
        public RequestValidator(TimetileDbContext db, IInstitutionProvider institutionProvider)
        {
            ...
            RuleFor(x => x.Id)
                ...
                {
                    RuleFor(x => x.Id)
                        .MustBeValidEntityId<GetSubmissionFilesEndpoint.Request, Submission>(db)
                        .DependentRules(() =>
                        {
                            RuleFor(x => x.Id)
