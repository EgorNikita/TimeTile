...
public static class RegexPatterns
{
    public static readonly Dictionary<string, (System.Text.RegularExpressions.Regex Pattern, short MaxLength, string Description)> Patterns = new()
    {
        ...
        ["Name"] = (new System.Text.RegularExpressions.Regex(@"^\p{L}+(?:[\s'-]\p{L}+)*$", RegexOptions.Compiled), 100, "Full names may include letters, apostrophes, hyphens, and spaces."),
        ["Address"] = (new System.Text.RegularExpressions.Regex(@"^[\p{L}\d\s'.,#/\-()]+$", RegexOptions.Compiled), 200, "Addresses may contain letters, numbers, spaces, and punctuation like '.,#/-()."),
        ...
        ["Domain"] = (new System.Text.RegularExpressions.Regex(@"^(?!-)(?:[\p{L}0-9-]{1,63}(?<!-)\.)+[A-Za-z]{2,}$", RegexOptions.Compiled), 255, "Domain must be valid and support international formats like sub.domain.com."),
        ["Description"] = (new System.Text.RegularExpressions.Regex(@"^[\p{L}\d\s.,!?]+$", RegexOptions.Compiled), 250, "Descriptions may include letters, numbers, spaces, and punctuation like .,!?"),
    };
}